            ...
            <div class="card p-3 sticky-top" style="top:80px">
                ...
                <form action="{{ route('catalog') }}" method="GET" id="filterForm">
                    ...
                    <div class="mb-3">
                        ...
                        <select name="liquid_type" class="form-select form-select-sm" onchange="document.getElementById('filterForm').submit()">
                            <option value="">Semua</option>
                            <option value="freebase" {{ request('liquid_type')=='freebase' ? 'selected':'' }}>Freebase</option>
                            <option value="salt" {{ request('liquid_type')=='salt' ? 'selected':'' }}>Salt Nic</option>
                            <option value="kosong" {{ request('liquid_type')=='kosong' ? 'selected':'' }}>Kosong</option>
                        </select>
                    </div>
                    <div class="mb-3">
                        <label class="form-label small fw-semibold">Harga Maks (Rp)</label>
                        <input type="number" name="max_price" class="form-control form-control-sm" value="{{ request('max_price') }}" placeholder="500000">
                    </div>
                    <div class="mb-3">
        ...
        <div class="col-lg-9">
                    ...
                    <div class="col-6 col-md-3">
                        <div class="card h-100 border-0 shadow-sm rounded-4" style="border-top:3px solid #ff6b35 !important;position:relative">
                            ...
                            <div class="card-body p-2">
                                <span class="badge rounded-pill mb-1" style="background:#f0f0f0;color:#666;font-size:.6rem">
                                    {{ $rec->category->name }}
                                </span>
                                <a href="{{ route('product.detail', $rec) }}" class="text-decoration-none text-dark">
                                    <p class="fw-semibold mb-1 lh-sm" style="font-size:.8rem">{{ $rec->name }}</p>
                                </a>
                                <div class="d-flex align-items-center gap-1 mb-1">
                                    <i class="bi bi-star-fill text-warning" style="font-size:.65rem"></i>
                                    <small class="text-muted" style="font-size:.65rem">{{ $rec->rating }} · {{ number_format($rec->purchase_count) }}x terjual</small>
                                </div>
                                <div class="fw-bold" style="color:#ff6b35;font-size:.85rem">{{ $rec->formatted_price }}</div>
                            </div>
                        </div>
                    </div>
                    @endforeach
            ...
            <div class="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
                ...
                <form action="{{ route('catalog') }}" method="GET" class="d-flex gap-2">
                    ...
                    @endforeach
                    <input type="text" name="search" class="form-control form-control-sm" placeholder="Cari produk..." value="{{ request('search') }}" style="min-width:200px">
                    ...
                    <div class="col-6 col-md-4">
                        <div class="card h-100 product-card">
                            ...
                            <div class="card-body d-flex flex-column p-3">
                                <span class="badge rounded-pill mb-1" style="background:#f0f0f0;color:#666;font-size:.65rem">
                                ...
                                <a href="{{ route('product.detail', $product) }}" class="text-decoration-none text-dark">
                                    <h6 class="fw-semibold mb-1" style="font-size:.9rem">{{ $product->name }}</h6>
                                </a>
                                ...
                                @auth
                                    ...
                                    @else
                                        <a href="{{ route('product.detail', $product) }}" class="btn btn-outline-secondary btn-sm w-100">Detail</a>
                                    @endif
                                @else
